...
/**
 * El metodo email para autentificacion solo aplicas para empresas.
 * El metodo de telefono solo aplica para afiliados y empresas en los metodos de autenficacion del AAD.
 * 
 */
...
public class AADService {
	...
	public String updateUser( JSONCSActualizaDato data ) throws Exception {
         ...
         * */
        if ( validaCampos > 0 ) { 
        	if(data.getRegistroAad().equals("S")) {
        		HttpResponse<String> jsonResponse = Unirest.post(uri).headers(headers).body(sb.toString()).asString();
    	        log.warn("respuest de Creacion de usuario de campos {}", jsonResponse.getBody() );
    	        
    	        String uriSendMail = oAuthProperties.getProperty("send_mail_new_user");
    	        log.warn("url envio de correo para usuario nuevo", uriSendMail );
    	        
    	        StringBuilder correoData = new StringBuilder();
    	        correoData.append("{\"nup\":\""
    	        		+
    	        		data.getNup()
    	        		+ "\",\"correo_electronico\":\""
    	        		+ data.getCorreo1New()
    	        ...
    	        HttpResponse<String> jsonResponseSendMail = Unirest.post(uriSendMail).headers(headers).body(sb.toString()).asString();
	...
	public String update(@Body String input)  {
		...
		if( respuesta == null ) {
			data.setValid( EEstado.ACTUALIZADO.ordinal()+"" );
		}else {
	...
	public String checkAuthenticationPhoneMethods(String id) {
   	...
   	String authenticationPhoneParam = graphAuthenticationPhoneUrl.replace("{id}", id); 
   	   ...
   	   log.warn("URL {checkAuthenticationPhoneMethods}",authenticationPhoneParam);
   	   
       HttpResponse<JsonNode> jsonResponse = Unirest.get(authenticationPhoneParam).headers(headers).asJson();
		log.warn("respuesta {checkAuthenticationPhoneMethods}", jsonResponse.getBody().getObject().get( VALUE ) );
   ...
   public String updateAuthenticationEmailMethods(String id, String email) {
	   try {
	    	...
	    	Map<String, String> headers = getHeaderGraph(tmpToken); 
	    	int lengJson = email.length()+24;
	    	headers.put("Content-Length:", lengJson+"" );
			...
			log.warn("respuesta updateAuthenticationEmailMethods {}", jsonResponse.getBody());
			log.warn("respuesta updateAuthenticationEmailMethods {}", jsonResponse.getHeaders());
   ...
   public String createAuthenticationEmailMethods(String id, String email) {
	   try {
	        ...
	        log.warn("respuesta {createAuthenticationEmailMethods}", jsonResponse.getBody());
   ...
   public String checkAuthenticationEmailMethods(String id) {
	   try {
	        ...
	        log.warn("respuesta {checkAuthenticationEmailMethods}", jsonResponse.getBody());
   ...
   public String createUser( JSONCSActualizaDato data ) throws Exception {
		log.info("updateUser - inicio");
		if(oAuthProperties==null) loadConfig();
