...
BEGIN
	BEGIN
		SELECT NUM_ID INTO v_num_id FROM RE.BFP_PERSONA WHERE NUM_ID = :NEW.COD_CLIENTE AND ROWNUM <=1;
	EXCEPTION
    ...
    END;
   	...
   	IF v_num_id IS NOT NULL THEN
   		BEGIN
			SELECT  ESTADO_AFILIADO INTO v_estado_afiliado  FROM  DUAL LEFT JOIN  RE.BFP_PERSONA ON num_id = :NEW.COD_CLIENTE AND ROWNUM <=1;
		EXCEPTION
	      WHEN NO_DATA_FOUND THEN
	   ...
	   IF v_estado_afiliado IS NOT NULL AND v_estado_afiliado = 'ACT'  THEN
	   		--CORREO ELECTRONICO
	   		BEGIN 
		   		SELECT CORREO_ELECTRONICO INTO v_correo_electronico FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		    ...
		    BEGIN 
		   		SELECT CORREO_ELECTRONICO2 INTO v_correo_electronico2 FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		    ...
		    --ESTADO_EMAIL1
		    BEGIN 
		   		SELECT ESTADO_EMAIL1 INTO v_estado_email1 FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   --ESTADO_EMAIL2
		    BEGIN 
		   		SELECT ESTADO_EMAIL2 INTO v_estado_email2 FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		  ...
		  --FECHA_EXPEDICION_ID
		    BEGIN 
		   		SELECT FECHA_EXPEDICION_ID INTO v_fecha_expedicion_id FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   --FECHA_EXPIRACION
		    BEGIN 
		   		SELECT FECHA_EXPIRACION INTO v_fecha_expiracion FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		    ...
		    IF f_modifica_correo = 1 AND  f_fecha_expedicion_id = 1 AND f_fecha_expiracion =1 THEN
		    	UPDATE BFP_PERSONA SET CORREO_ELECTRONICO = :NEW.CORREO_ELECTRONICO, FECHA_EXPEDICION_ID = :NEW.DOC_DATEOFISSUE,  FECHA_EXPIRACION = :NEW.DOC_DATEOFEXPIRY, ETAPA_INFO_ELECT = 4  WHERE  num_id = :NEW.COD_CLIENTE;
		    	COMMIT;
		    END IF;
		   
		    IF f_modifica_correo = 0 AND  f_fecha_expedicion_id = 1 AND f_fecha_expiracion =1 THEN
		    	UPDATE BFP_PERSONA SET  FECHA_EXPEDICION_ID = :NEW.DOC_DATEOFISSUE,  FECHA_EXPIRACION = :NEW.DOC_DATEOFEXPIRY, ETAPA_INFO_ELECT = 4  WHERE  num_id = :NEW.COD_CLIENTE;
		    	COMMIT;
		    END IF;
		   
		    IF f_modifica_correo = 0 AND  f_fecha_expedicion_id = 0 AND f_fecha_expiracion =1 THEN
		    	UPDATE BFP_PERSONA SET    FECHA_EXPIRACION = :NEW.DOC_DATEOFEXPIRY, ETAPA_INFO_ELECT = 4  WHERE  num_id = :NEW.COD_CLIENTE;
		    	COMMIT;
		    END IF;
		  ...
		  --PREPARADO PARA INSERT EN DB INTERMEDIA AD
		   ...
		   BEGIN 
		   		SELECT TIPO_ID INTO v_TIPO_DOCUMENTO_AD FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   BEGIN 
		   		SELECT NIT_AFILIADO INTO v_NIT FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		        v_NIT := NULL;
		      WHEN OTHERS THEN
		    ...
		    BEGIN 
		   		SELECT NUM_ID INTO v_NUM_ID FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		        v_NUM_ID := NULL;
		      WHEN OTHERS THEN
		   ...
		   BEGIN 
		   		SELECT COD_EMPRESA INTO v_COD_EMPRESA FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		        v_COD_EMPRESA := NULL;
		      WHEN OTHERS THEN
		   ...
		   BEGIN 
		   		SELECT PRIMER_NOMBRE INTO v_PRIMER_NOMBRE_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		    ...
		    BEGIN 
		   		SELECT SEGUNDO_NOMBRE INTO v_SEGUNDO_NOMBRE_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		    ...
		    BEGIN 
		   		SELECT PRIMER_APELLIDO  INTO v_PRIMER_APELLIDO_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   BEGIN 
		   		SELECT SEGUNDO_APELLIDO  INTO v_SEGUNDO_APELLIDO_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   BEGIN 
		   		SELECT APELLIDO_CASADA  INTO v_APELLIDO_CASADA_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   BEGIN 
		   		SELECT TIPO_ID INTO v_TIPO_DOCUMENTO_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   BEGIN 
		   		SELECT NUP  INTO v_NUMERO_DOCUMENTO_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		   ...
		   BEGIN 
		   		SELECT FECHA_NACIMIENTO  INTO v_FECHA_NACIMIENTO_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		    ...
		    BEGIN 
		   		SELECT CORREO_ELECTRONICO  INTO v_CORREO1_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		        v_CORREO1_NEW := NULL;
		      WHEN OTHERS THEN
		   ...
		   BEGIN 
		   		SELECT CORREO_ELECTRONICO2  INTO v_CORREO2_NEW FROM RE.BFP_PERSONA WHERE num_id = :NEW.COD_CLIENTE;
		   	EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		        v_CORREO2_NEW := NULL;
		      WHEN OTHERS THEN
