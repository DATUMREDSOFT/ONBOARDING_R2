...
BEGIN
    ...
    IF V_NUM_ID IS NOT NULL THEN
        ...
        IF V_ESTADO_AFILIADO = 'ACT' THEN          
            SELECT CORREO_ELECTRONICO INTO V_CORREO_ELECTRONICO FROM DUAL LEFT JOIN  BFP_PERSONA ON num_id = :NEW.COD_CLIENTE;
            SELECT CORREO_ELECTRONICO2 INTO V_CORREO_ELECTRONICO2 FROM DUAL LEFT JOIN  BFP_PERSONA ON num_id = :NEW.COD_CLIENTE;
            SELECT ESTADO_EMAIL1 INTO V_ESTADO_EMAIL1 FROM DUAL LEFT JOIN  BFP_PERSONA ON num_id = :NEW.COD_CLIENTE;
            SELECT ESTADO_EMAIL2 INTO V_ESTADO_EMAIL2 FROM DUAL LEFT JOIN  BFP_PERSONA ON num_id = :NEW.COD_CLIENTE;  
            IF  V_CORREO_ELECTRONICO IS NULL OR :NEW.CORREO_ELECTRONICO != V_CORREO_ELECTRONICO THEN
                IF V_CORREO_ELECTRONICO2 IS NULL OR :NEW.CORREO_ELECTRONICO != V_CORREO_ELECTRONICO2 THEN                  
                    IF V_ESTADO_EMAIL1 IS NOT NULL AND V_ESTADO_EMAIL1 = 'A'  OR  V_ESTADO_EMAIL2 IS NOT NULL AND V_ESTADO_EMAIL2 = 'A'THEN       
                        UPDATE BFP_PERSONA SET CORREO_ELECTRONICO = :NEW.CORREO_ELECTRONICO WHERE  num_id = :NEW.COD_CLIENTE;                      
                    END IF;
                END IF;
            END IF;
        END IF;
